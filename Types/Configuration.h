...
class Configuration
{
  ...
  void Configure(const std::string config_filename){
    ...
    while(std::getline(config_file, line)){
      ...
      if(key.find("TreePath") != std::string::npos)     tree_path = TString(value);
      if(key.find("MetaDataPath") != std::string::npos) metadata_path = TString(value);
      if(key.find("WeightPath") != std::string::npos)   weight_path = TString(value);
      ...
      if(key.find("ShowPlots") != std::string::npos)    show_plots = ToBools(value);
  ...
  void GetMetaData(){
    ...
    for(size_t i = 0; i < input_file.size(); i++){
      ...
      //Read in TTree
      TTreeReader tree_reader(metadata_path, &data_file);
